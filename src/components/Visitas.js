...
const Visitas = () => {
  ...
  const visitsContainerRef = useRef(null); // Ref para el elemento visitas-container
...
/*
 ...
 /* function setDark(val){
  ...
  const handleCheckChange = (chain, day) => {
    setVisits(prevVisits => {
      const personVisits = prevVisits[selectedPerson] || {};
      const chainVisits = personVisits[chain] || {};
      return {
        ...prevVisits,
        [selectedPerson]: {
          ...personVisits,
  ...
  const generatePNG = () => {
    const element = visitsContainerRef.current; // Obtener el elemento con la referencia
    if (!element) return; // Verificar que el elemento exista
  ...
  const handlePasswordSubmit = () => {
    ...
    switch (password) {
      case 'santi91':
        setSelectedPerson('Santiago Ruiz');
        break;
      case 'marce15':
        setSelectedPerson('Marcelo Carcar');
        break;
      case 'jorge38':
        setSelectedPerson('Jorge Virgilio');
        break;
      case 'cris33':
        setSelectedPerson('Cristian Villagra');
        break;
      case 'lauti21':
        setSelectedPerson('Lautaro Galli');
        break;
      case '0716':
        setSelectedPerson(''); // Si se ingresa el código de contraseña "0716", mostrar nuevamente el selector original de personas
        break;
      default:
  ...
  if (showPasswordPrompt) {
    return (
      <div className="flex flex-col items-center bg-gray-200 h-full justify-center">
        <div className="w-full bg-blue-200 bg-opacity-25 max-w-xs mt-4">
          <form className="bg-blue-200 bg-opacity-25 shadow-md rounded px-4 pt-6 pb-8">
            <div className="mb-4 text-center">
              <FontAwesomeIcon icon={faUser} className="text-2xl text-gray-800 mb-2" />
              <input
                type="password"
                id="password"
                value={password}
...
<body class = "body bg-white dark:bg-[#0F172A]">
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {username && <p className="text-xl">Bienvenido, {username}!</p>}
      {selectedPerson ? (
        <div className=" justify-center items-center ">
          <div ref={visitsContainerRef} className="overflow-x-auto w-full">
            <table className="min-w bg-white">
              <thead>
                <tr>
                  <th className="py-2 px-2 border-b">Tienda</th>
                  {daysOfWeek.map(day => (
                    <th key={day} className="py-2 px-2 border-b">{day}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {Object.keys(data[selectedPerson]).map(chain => (
                  <tr key={chain}>
                    <td className="py-2 px-4 border-b">{chain}</td>
                    {daysOfWeek.map(day => (
                      <td key={day} className="py-2 px-2 border-b text-right">
                        <div
                          className={`rounded-full h-3 w-3 ${
                            visits[selectedPerson]?.[chain]?.[day] ? 'bg-blue-600' : 'bg-gray-300'
                          } flex items-center justify-center cursor-pointer`}
                          onClick={() => handleCheckChange(chain, day)}
                        >
                          {visits[selectedPerson]?.[chain]?.[day] && (
        ...
        <PersonSelector
          persons={Object.keys(data)}
          onSelectPerson={setSelectedPerson}
        />
      )}
      <button
        className="py-2 px-4 bg-blue-500 text-white rounded-lg shadow-md mt-4"
        onClick={generatePNG}
      >
        Guardar ruteo
      </button>
